...
fun <R> tryOut(action: () -> R) = runCatching { action() }.toWellOrUnfit().mapUnfit { "tried but ${it.message}" }
fun <R> Collection<() -> R>.tryOut() = runCatching { this.forEach { it.invoke() } }.toWellOrUnfit().mapUnfit { "tried but ${it.message}" }

fun <R> Collection<() -> R>.tryOutAndCombine(): WellsAndUnfits<Unit, String> =
  this.map { runCatching { this.forEach { it.invoke() } }.toWellOrUnfit().mapUnfit { "tried but ${it.message}" } }.combine()
...
data class WellsAndUnfits<R, U>(
  ...
  fun firstWellIfAllWell() = if (unfits.reason.isEmpty()) Well(wells.value.first()) else null
...
private fun <R, U> Collection<WellOrUnfit<R, U>>.combineToPair(): Pair<Well<List<R>>, Unfit<List<U>>> {
  ...
  this.forEach { result -> result.onUnfit { unfit: U -> unfitStrings.add(unfit) }.onWell { well -> results.add(well) } }
